...
test_that("generate_crosstab generates cross-tabulation correctly using different inputs", {
  ...
  expect_equal(attributes(df$category)$label, 'sex')
  expect_equal(attributes(df_l$category)$label, 'Sex')
  expect_equal(attributes(df_f$category)$label, 'sex')
  expect_equal(attributes(df2$category)$label, 'type')
  expect_equal(attributes(df2_l$category)$label, 'Type')
  expect_equal(attributes(df2_f$category)$label, 'type')
