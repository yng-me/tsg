...
#' @param wb A workbook object from \code{openxlsx} package.
#' @param sheet worksheet (sheet name/number) of the workbook instance.
#' @param title Table title.
#' @param description Table description.
#' @param options Formatting options.
#' @param start_col Column position to start write the data.
#' @param footnote Table footnote.
#' @param ... Column separator that defines the table hierarchy.
#'
...
tse_write_excel <- function(
  ...
  openxlsx::addWorksheet(wb = wb, sheet = sheet, gridLines = F)
  ...
  merge_colnames <- tsg_util_extract_col(
    data,
    start_col = start_col,
    start_row = start_row,
    ...
    ...
    if(row_depth_inner > 1) {
      ...
      for(j in 2:row_depth_inner) {
        ...
        inner_seq <- tibble::as_tibble_col(
          tsg_util_increment_val(inner$value),
          column_name = 'seq'
        )
